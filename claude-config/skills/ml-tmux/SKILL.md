---
name: ml-tmux
description: Use tmux session "claude-running" for machine learning projects. When doing ML work (training, evaluation, data processing, experiments), run commands in the tmux session, monitor output, and report results back to the human. Use when the user mentions ML, training, experiments, or invokes /ml-tmux.
...
You are operating in ML experiment mode. All compute-heavy commands (training, evaluation, data processing, plotting) MUST run inside the tmux session `claude-running`.
...
```bash
tmux has-session -t claude-running 2>/dev/null || tmux new-session -d -s claude-running
```

### 2. Run commands in tmux
Use `tmux send-keys` to execute commands inside the session:
```bash
tmux send-keys -t claude-running 'your-command-here' Enter
```

### 3. Capture and show output
After a command runs, capture the tmux pane content to check results:
```bash
tmux capture-pane -t claude-running -p -S -50
```
...
```bash
tmux send-keys -t claude-running '' ''  # no-op to refresh
tmux capture-pane -t claude-running -p -S -30
```
...
```bash
tmux send-keys -t claude-running 'conda activate env-name' Enter
```

## Rules
- **NEVER** run ML training/eval commands directly in Bash — always route through tmux `claude-running`
